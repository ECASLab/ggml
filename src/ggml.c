...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor, struct ggml_compute_state * state) {
    ...
    switch (tensor->op) {
        ...
        if (tensor->src1 != NULL)
        {
            printf("DName: %c, SName: %c, S2Name: %c, OpName: %c, d1: %d, d2: %d, d3: %d, d4: %d, sp1: %d, sp2: %d, sp3: %d, sp4: %d, 
            ss1: %d, ss2: %d, ss3: %d, ss4: %d", tensor->name, tensor->src0->name, tensor->src1->name, ggml_op_name(tensor->op), tensor->ne[0],
            tensor->ne[1], tensor->ne[2], tensor->ne[3], tensor->src0->ne[0], tensor->src0->ne[1], tensor->src0->ne[2], tensor->src0->ne[3], 
            tensor->src1->ne[0], tensor->src1->ne[1], tensor->src1->ne[2], tensor->src1->ne[3]);
        } else {
            printf("DName: %c, SName: %c, S2Name: %c, OpName: %c, d1: %d, d2: %d, d3: %d, d4: %d, sp1: %d, sp2: %d, sp3: %d, sp4: %d", 
            tensor->name, tensor->src0->name, tensor->src1->name, ggml_op_name(tensor->op), tensor->ne[0], tensor->ne[1],
